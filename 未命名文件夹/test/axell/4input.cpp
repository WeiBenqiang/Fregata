...
int main()
{
    ...
    for (int i = 0; i < num_test; i++)
        ca[i] = tlweSymEncrypt<lvl1param>(
            pa[i] ? lvl1param::mu / 2 : -(lvl1param::mu / 2), lvl1param::α,
            sk->key.lvl1);
    for (int i = 0; i < num_test; i++)
        cb[i] = tlweSymEncrypt<lvl1param>(
            pb[i] ? lvl1param::mu / 2 : -(lvl1param::mu / 2), lvl1param::α,
            sk->key.lvl1);
    for (int i = 0; i < num_test; i++)
        cc[i] = tlweSymEncrypt<lvl1param>(
            pc[i] ? lvl1param::mu / 2 : -(lvl1param::mu / 2), lvl1param::α,
            sk->key.lvl1);
    for (int i = 0; i < num_test; i++)
        cd[i] = tlweSymEncrypt<lvl1param>(
            pd[i] ? lvl1param::mu / 2 : -(lvl1param::mu / 2), lvl1param::α,
            sk->key.lvl1);
