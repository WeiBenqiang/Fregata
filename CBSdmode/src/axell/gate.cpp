             ...
             const TLWE<lvl1param> &cb, const EvalKey &ek)
{
    ...
    intemp[lvl1param::k * lvl1param::n] += 2 * lvl1param::mu;
    ...
    intemp[lvl1param::k * lvl1param::n] += lvl1param::mu;
    IdentityKeySwitch<lvl10param>(intemplvl0, intemp, *ek.iksklvl10);
                  ...
                  const EvalKey &ek)
{
    ...
    BlindRotate<lvl01param>(trlwe, cadd, *ek.bkfftlvl01, mupolygen<P, P::mu>());
    TLWE<P> cor;
                     ...
                     const TLWE<lvl1param> &cc, const EvalKey &ek)
{
    ...
    // carry
    GateBootstrappingTLWE2TLWEFFT<lvl01param>(
        carry, intemplvl0, *ek.bkfftlvl01, mupolygen<lvl1param, lvl1param::mu>());
    // sum
    for (int i = 0; i <= lvl0param::n; i++) intemplvl0[i] *= -2;
    GateBootstrappingTLWE2TLWEFFT<lvl01param>(
        sum, intemplvl0, *ek.bkfftlvl01, mupolygen<lvl1param, lvl1param::mu>());
}
...
constexpr Polynomial<P> xortestvecgen()
{
    ...
    for (int i = 0; i < P::n / 4; i++) {
        xortestvec[i] = -P::mu;
        xortestvec[i + P::n / 4] = P::mu;
        xortestvec[i + P::n / 2] = -P::mu;
        xortestvec[i + 3 * P::n / 4] = P::mu;
    }
    return xortestvec;
}
                  ...
                  const EvalKey &ek)
{
    ...
    res[lvl1param::k * lvl1param::n] += 2 * lvl1param::mu;
    TLWE<lvl0param> temp;
    IdentityKeySwitch<lvl10param>(temp, res, *ek.iksklvl10);
    TRLWE<lvl1param> rtemp;
    BlindRotate<lvl01param>(rtemp, temp, *ek.bkfftlvl01,
...
void Hom3inputThreashold(TLWE<lvl1param> &res, const TLWE<lvl1param> &ca,
                         ...
                         const EvalKey &ek)
{
    res[lvl1param::k * lvl1param::n] += lvl1param::mu / 2;
    for (int i = 0; i <= lvl1param::k * lvl1param::n; i++) res[i] = ca[i] + cb[i] + cc[i];
    TLWE<lvl0param> temp;
    IdentityKeySwitch<lvl10param>(temp, res, *ek.iksklvl10);
    GateBootstrappingTLWE2TLWEFFT<lvl01param>(
        res, temp, *ek.bkfftlvl01, mupolygen<lvl1param, lvl1param::mu>());
}
...
{
    Polynomial<P> testvec;
    for (int i = 0; i < P::n; i++) testvec[i] = -P::mu;
    return testvec;
}
