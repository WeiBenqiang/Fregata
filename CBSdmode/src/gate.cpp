                    ...
                    const EvalKey &ek)
{
    ...
    res[P::k * P::n] += offset;
    GateBootstrapping<P::mu>(res, res, ek);
}
...
void HomNAND(TLWE<P> &res, const TLWE<P> &ca, const TLWE<P> &cb,
             const EvalKey &ek)
{
    HomGate<P, -1, -1, P::mu>(res, ca, cb, ek);
}
#define INST(P)                                                \
             ...
             const EvalKey &ek)
{
    if constexpr (std::is_same_v<P, lvl1param> ||
        ...
        HomGate<P, -2, -2, -2 * lvl1param::mu>(res, ca, cb, ek);
    }
    else if constexpr (std::is_same_v<P, lvlMparam>) {
        HomGate<P, -3, -3, -3 * lvlMparam::mu>(res, ca, cb, ek);
    }
}
#define INST(P)                                                \
                ...
                const TLWE<typename iksP::domainP> &c0, const EvalKey &ek)
{
    ...
    temp1[iksP::domainP::n] -= iksP::domainP::mu;
    temp0[iksP::domainP::n] -= iksP::domainP::mu;
    TLWE<lvl0param> and1, and0;
