 ...
 * @LastEditTime: 2023-02-07 20:01:35
 * @Description: 
 * @FilePath: /tfhepp/test/combinedmem.cpp
 */
#include <bitset>
#include <cassert>
#include <chrono>
    ...
    for (uint8_t wrflag = 0; wrflag <= 1; wrflag++) {
        for (uint8_t msbaddress = 0; msbaddress <= 1; msbaddress++) {
            ...
            encwrflag = tlweSymEncrypt<lvl1param>(
                (wrflag > 0) ? lvl1param::mu : -lvl1param::mu, lvl1param::alpha,
                (*sk).key.lvl1);

            for (int i = 0; i < words; i++)
                encwritep[i] = tlweSymEncrypt<lvl1param>(
                    writep[i] ? lvl1param::mu : -lvl1param::mu, lvl1param::alpha,
                    (*sk).key.lvl1);
