    ...
    for (int i = 0; i < num_test; i++)
        for(int j = 0; j<2; j++){
            ca[i][j] = TFHEpp::tlweSymEncrypt<TFHEpp::lvl1param>(
                (pa[i]>>j)&1 ? TFHEpp::lvlMparam::mu : -TFHEpp::lvlMparam::mu,
                TFHEpp::lvl1param::α, sk->key.lvl1);
            TFHEpp::GateBootstrapping<TFHEpp::lvlMparam::mu>(ca[i][j],ca[i][j],ek);
        }
    for (int i = 0; i < num_test; i++)
        for(int j = 0; j<2; j++){
            cb[i][j] = TFHEpp::tlweSymEncrypt<TFHEpp::lvl1param>(
                (pb[i]>>j)&1 ? TFHEpp::lvlMparam::mu : -TFHEpp::lvlMparam::mu,
                TFHEpp::lvl1param::α, sk->key.lvl1);
            TFHEpp::GateBootstrapping<TFHEpp::lvlMparam::mu>(cb[i][j],cb[i][j],ek);
        }
